...
import type { FieldType, SectionGroup } from '../../types'
      ...
      {sections.map((sec) => (
        <Card key={sec.id}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div title="把區塊歸到文設定／圖設定分頁" style={{ display: 'flex', gap: 3, background: 'var(--bg-3)', padding: 3, borderRadius: 9, flexShrink: 0 }}>
              {(['text', 'image'] as SectionGroup[]).map((g) => (
                <button
                  key={g}
                  onClick={() => updateSection(sec.id, 'group', g)}
                  style={{
                    fontFamily: 'inherit',
                    fontSize: 11.5,
                    fontWeight: 700,
                    ...
                    borderRadius: 7,
                    padding: '5px 10px',
                    background: (sec.group || 'text') === g ? 'var(--accent)' : 'transparent',
                    color: (sec.group || 'text') === g ? '#fff' : 'var(--text-2)',
                  }}
                >
                  {g === 'text' ? '文設定' : '圖設定'}
                </button>
                ...
                <div key={fd.id} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 7 }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <input
                        value={fd.label}
                        onChange={(e) => updateField(sec.id, fd.id, 'label', e.target.value)}
                        placeholder="欄位名稱"
                        style={{
                          flex: 1,
                          minWidth: 0,
                      ...
                      <select
                        value={fd.type}
                        onChange={(e) => updateField(sec.id, fd.id, 'type', e.target.value)}
                        style={{
                          flexShrink: 0,
                          fontFamily: 'inherit',
                          fontSize: 12.5,
                    ...
                    {multi ? (
                      <textarea
                        value={fd.value}
                        onChange={(e) => updateField(sec.id, fd.id, 'value', e.target.value)}
                        placeholder="輸入內容…"
                        style={{ ...fieldInput, minHeight: 80, resize: 'vertical', lineHeight: 1.7 }}
                      />
                    ) : (
                      <input
                        value={fd.value}
                        onChange={(e) => updateField(sec.id, fd.id, 'value', e.target.value)}
                        placeholder={
                          fd.type === 'tags' || fd.type === 'check' || fd.type === 'avoid'
                            ? '用、分隔多個項目'
                            : '輸入內容…'
                        }
                        style={fieldInput}
                  ...
                  <button
                    onClick={() => removeField(sec.id, fd.id)}
                    style={{
                      flexShrink: 0,
                      width: 32,
                      height: 32,
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 14 }}>
            <button style={addBtn} onClick={() => addField(sec.id, 'text')}>
              ＋ 短文字
            </button>
            <button style={addBtn} onClick={() => addField(sec.id, 'longtext')}>
              ＋ 長文字
            </button>
            <button style={addBtn} onClick={() => addField(sec.id, 'tags')}>
              ＋ 標籤
            </button>
            <button style={addBtn} onClick={() => addField(sec.id, 'check')}>
              ＋ 必畫重點 ✓
            </button>
            <button style={addBtn} onClick={() => addField(sec.id, 'avoid')}>
              ＋ 不可畫錯 ✕
            </button>
          </div>
        </Card>
