...
export function ImagesCard() {
  const { character, updateCore, uploadCore, openCropper } = useOctool()
          ...
          <div key={f.key} style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', gap: 7 }}>
              <input
                ...
                onChange={(e) => updateCore(f.key, e.target.value)}
                placeholder="https://…"
                style={{
                  flex: 1,
                  minWidth: 0,
              ...
              />
              <button
                onClick={() => (f.crop ? openCropper(f.key) : uploadCore(f.key))}
                style={{
                  flexShrink: 0,
                  fontFamily: 'inherit',
                  fontSize: 12.5,
...
export function PaletteCard() {
  const { character, addSwatch, updateSwatch, removeSwatch, applyPreset, openEyedrop } = useOctool()
  return (
    <Card>
      <CardHead
        icon={<Icon name="palette" size={15} />}
        title="色票"
        right={
          <div style={{ display: 'flex', gap: 7 }}>
            ...
            <button style={softBtn()} onClick={addSwatch}>
              <Icon name="plus" size={14} /> 顏色
            </button>
          </div>
        }
      />
          ...
          >
            <span style={{ display: 'inline-flex', gap: 2 }}>
              {p.colors.map((c, i) => (
                <span
                  key={i}
                  style={{ width: 10, height: 10, borderRadius: '50%', background: c.hex }}
                />
              ))}
            </span>
            {p.name}
