...
import { BLOCK_TYPES, INFO_TYPES, PRESET_CARDS, TYPE_LABEL, findParentArr } from '../../data/blocks'
import type { Block, BlockType, DeviceKey } from '../../types'
...
// First-screen heights reflecting each device's real screen proportion.
// Desktop uses the live browser height (computed at render).
...
export function DesignPage() {
  ...
  // measure rendered content height for fold guides
  useEffect(() => {
  ...
  const cap = DEVICE_CAP[ui.device]
  const foldH = ui.device === 'desktop' ? winH : FOLD_H[ui.device]
              ...
              <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', padding: '11px 2px 4px' }}>
                {list.map((bt) => (
                  <button
                    ...
                    style={{ fontFamily: 'inherit', fontSize: 12.5, fontWeight: 600, color: 'var(--text)', background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: 9, padding: '7px 12px 7px 10px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}
                  >
  ...
  const noSelPanel = (
    <>
      ...
      {panelTab === 'design' ? <GlobalDesignPanel /> : panelTab === 'list' ? <BlockListPanel /> : addPanel}
    </>
  )
  ...
  return (
    <div>
      ...
      {/* template toolbar */}
      <div className="octstrip" style={{ display: 'flex', gap: 9, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
        ...
        {activeTemplate ? (
          <>
            <button style={{ fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'var(--text-2)', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 13px', cursor: 'pointer' }} onClick={() => duplicateTemplate(activeTemplate.id)}>
              複製
            </button>
            <button style={{ fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: '#c0584f', background: 'var(--bg-2)', border: '1px solid #e0b3ad', borderRadius: 10, padding: '8px 13px', cursor: 'pointer' }} onClick={() => removeTemplate(activeTemplate.id)}>
              刪除
            </button>
          </>
        ) : null}
      ...
      {pageCount > 1 ? (
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12, background: 'var(--bg-3)', padding: 5, borderRadius: 12, width: 'fit-content' }}>
          ...
          {Array.from({ length: pageCount }).map((_, i) => (
            <button key={i} style={seg(ui.pageView === i)} onClick={() => setPageView(i)}>
              第 {i + 1} 頁
            </button>
          ))}
        </div>
          ...
          {!mobile ? (
            <aside style={{ width: 320, flexShrink: 0 }}>
              <div style={{ position: 'sticky', top: 84, background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 18, padding: 18, maxHeight: 'calc(100vh - 110px)', overflowY: 'auto' }}>
                {selected ? <BlockInspector block={selected} /> : noSelPanel}
              </div>
            </aside>
          ) : null}
        </div>
      ...
      {isEdit && mobile && sheet !== 'none' ? (
        <>
          ...
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 61, background: 'var(--bg-2)', borderRadius: '18px 18px 0 0', boxShadow: '0 -12px 36px rgba(0,0,0,0.25)', maxHeight: '76vh', overflowY: 'auto', padding: 18 }}>
            ...
            {sheet === 'inspect' && selected ? <BlockInspector block={selected} /> : noSelPanel}
          </div>
        </>
      ) : null}
      ...
      {/* fullscreen presentation (paginated) */}
      {fs ? (
        <div style={{ position: 'fixed', inset: 0, zIndex: 90, background: activeTemplate?.design?.bg || '#fff', overflowY: 'auto' }}>
          <button onClick={() => setFs(false)} title="關閉 (Esc)" style={{ position: 'fixed', top: 18, right: 20, width: 42, height: 42, borderRadius: 12, border: 'none', background: 'rgba(0,0,0,0.22)', color: '#fff', cursor: 'pointer', fontSize: 22, zIndex: 3 }}>
            ×
          </button>
          <TemplateCanvas character={character} template={activeTemplate} annotateMode={ui.annotateMode} viewport={ui.device} pageView={pageCount > 1 ? fsPage : 'all'} onNavTo={(p) => setFsPage(p)} />
          {pageCount > 1 ? (
            <div style={{ position: 'fixed', left: '50%', bottom: 24, transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 14, background: 'rgba(0,0,0,0.34)', backdropFilter: 'blur(8px)', borderRadius: 999, padding: '8px 14px', zIndex: 3 }}>
              ...
              <span style={{ fontFamily: "'Nunito Sans'", fontSize: 13, fontWeight: 700, color: '#fff', minWidth: 46, textAlign: 'center' }}>{fsPage + 1} / {pageCount}</span>
