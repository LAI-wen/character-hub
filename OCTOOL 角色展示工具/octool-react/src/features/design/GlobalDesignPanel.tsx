...
import { FONT_OPTS } from '../../data/themes'
import { pickImage } from '../../data/upload'
import type { Design, DeviceKey } from '../../types'

const label = { fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 6 } as const
const selectStyle = {
  width: '100%',
...
const DEV: { k: DeviceKey; l: string; key: 'bgImage' | 'bgImageTablet' | 'bgImagePhone'; hint: string }[] = [
...
function Swatch({ hex, sel, onClick }: { hex: string; sel: boolean; onClick: () => void }) {
...
function Section({ id, icon, title, open, onToggle, children }: { id: string; icon: string; title: string; open: boolean; onToggle: (id: string) => void; children: ReactNode }) {
  return (
    <div>
      <button
        onClick={() => onToggle(id)}
        style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', cursor: 'pointer', background: 'var(--bg-2)', border: open ? '1px solid var(--accent)' : '1px solid var(--border)', borderRadius: 12, padding: '12px 14px', fontFamily: 'inherit', fontSize: 14, fontWeight: 700, color: 'var(--text)' }}
      >
...
export function GlobalDesignPanel() {
  const { character, activeTemplate, updateDesign, libColors, addLibColor, removeLibColor } = useOctool()
  const d: Design = activeTemplate?.design || {}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <Section id="bg" icon="image" title="背景設定" open={open === 'bg'} onToggle={toggle}>
        <div>
          <div style={label}>背景圖（可分裝置）</div>
          <div style={{ display: 'flex', gap: 4, background: 'var(--bg-3)', padding: 4, borderRadius: 10 }}>
            {DEV.map((x) => (
              <button key={x.k} style={seg(bgDev === x.k)} onClick={() => setBgDev(x.k)}>{x.l}</button>
            ))}
          </div>
          ...
          <button onClick={() => pickImage((url) => updateDesign(dev.key, url))} style={{ width: '100%', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, color: 'var(--text-2)', background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: 10, padding: '11px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <Icon name="upload" size={14} /> 上傳背景圖
          </button>
          <div style={{ ...label, marginTop: 10 }}>或貼上圖片 URL</div>
          <input value={curBg} onChange={(e) => updateDesign(dev.key, e.target.value)} placeholder="https://…" style={textInput} />
          {curBg ? (
            <button onClick={() => updateDesign(dev.key, '')} style={{ marginTop: 7, fontFamily: 'inherit', fontSize: 12, fontWeight: 600, color: 'var(--text-2)', background: 'transparent', border: '1px solid var(--border)', borderRadius: 9, padding: '6px 12px', cursor: 'pointer' }}>移除此裝置背景</button>
          ) : null}
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, alignItems: 'center' }}>
            {swatchLib.map((hex) => (
              <Swatch key={hex} hex={hex} sel={(d.bg || '#ffffff') === hex} onClick={() => updateDesign('bg', hex)} />
            ))}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <div style={label}>重複</div>
            <select value={d.bgRepeat || 'no-repeat'} onChange={(e) => updateDesign('bgRepeat', e.target.value)} style={selectStyle}>
              {BG_REPEAT.map((o) => (<option key={o.v} value={o.v}>{o.l}</option>))}
            </select>
          </div>
          <div style={{ flex: 1 }}>
            <div style={label}>填充</div>
            <select value={d.bgSize || 'cover'} onChange={(e) => updateDesign('bgSize', e.target.value)} style={selectStyle}>
              {BG_SIZE.map((o) => (<option key={o.v} value={o.v}>{o.l}</option>))}
            </select>
          </div>
        </div>
        <div>
          <div style={label}>圖片捲動</div>
          <select value={d.bgAttach || 'scroll'} onChange={(e) => updateDesign('bgAttach', e.target.value)} style={selectStyle}>
            {BG_ATTACH.map((o) => (<option key={o.v} value={o.v}>{o.l}</option>))}
          </select>
        </div>
        <div>
          <div style={label}>亮度 {d.maskBright ?? 100}%</div>
          <input type="range" min={20} max={150} value={d.maskBright ?? 100} onChange={(e) => updateDesign('maskBright', Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--accent)' }} />
        </div>
        <div>
          <div style={label}>模糊 {d.maskBlur ?? 0}px ・ 飽和 {d.maskSat ?? 100}%</div>
      ...
      <Section id="font" icon="type" title="字型與主色" open={open === 'font'} onToggle={toggle}>
        ...
        <div>
          <div style={label}>主色</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, alignItems: 'center' }}>
            {swatchLib.map((hex) => (
              <Swatch key={hex} hex={hex} sel={(d.primary || '#c98a5e') === hex} onClick={() => updateDesign('primary', hex)} />
            ))}
            <label style={{ width: 30, height: 30, borderRadius: 9, border: '1px dashed var(--border)', background: 'var(--bg-2)', position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-2)', cursor: 'pointer', flexShrink: 0 }}>
              <Icon name="droplet" size={14} />
      ...
      <Section id="width" icon="layout" title="佈局寬度" open={open === 'width'} onToggle={toggle}>
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg-3)', padding: 4, borderRadius: 10 }}>
          {WIDTH_OPTS.map((o) => (
            <button key={o.key} style={seg((d.width || 'normal') === o.key)} onClick={() => updateDesign('width', o.key)}>{o.label}</button>
          ))}
        </div>
        <div>
          <div style={label}>每頁高度（全螢幕翻頁）</div>
          <div style={{ display: 'flex', gap: 4, background: 'var(--bg-3)', padding: 4, borderRadius: 10 }}>
