...
import type { Album, AlbumImage, Annotation, Block, Character, Design, Field, Section } from '../types'

// ============================================================================
// TemplateCanvas — full-fidelity port of TemplateCanvas.dc.html.
// Renders a character through a template's block list, with editing
// affordances (selection, drag-reorder, column resize) when `editable`.
// ============================================================================
...
export function TemplateCanvas(props: CanvasProps) {
  ...
  // ---- section field ----
  function fieldEl(f: Field, sec: Section, cards: boolean): ReactNode {
  ...
  function buildContent(b: Block): ReactNode {
    ...
    switch (b.type) {
      ...
      case 'marquee': {
        ...
        const spd = (b as { speed?: number }).speed || 18
        const seg = (k: string) => (
  ...
  function BlockEl({ b, nested = false }: { b: Block; nested?: boolean }): ReactNode {
    ...
    return (
      <div
        ...
        data-bid={b.id}
        draggable={editable}
        onClick={editable ? (e) => { e.stopPropagation(); onSelect && onSelect(b.id) } : undefined}
        onDragStart={
          editable
            ? (e) => {
                e.stopPropagation()
  ...
  // 固定一屏：showing a single page → each page fills one screen, centred.
