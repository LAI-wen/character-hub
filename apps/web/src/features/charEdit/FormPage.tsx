    ...
    <div style={{ display: 'flex', gap: 26, alignItems: 'flex-start' }}>
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ fontFamily: "'Newsreader', serif", fontWeight: 600, fontSize: 26, margin: 0, color: 'var(--text)' }}>編輯角色</h1>
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            ...
            <button style={{ ...ghostBtn(), color: '#c0584f', borderColor: '#e0b3ad' }} onClick={clearAll}><Icon name="trash" size={14} /> 清空</button>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 5, background: 'var(--bg-3)', padding: 5, borderRadius: 14, flexWrap: 'wrap', alignSelf: 'flex-start' }}>
          {TABS.map((t) => {
            const active = tab === t.key
            return (
              <button key={t.key} onClick={() => setFormTab(t.key)}
                style={{ fontFamily: 'inherit', fontSize: 13.5, fontWeight: 700, border: 'none', cursor: 'pointer', borderRadius: 10, padding: '8px 15px', background: active ? 'var(--bg-2)' : 'transparent', color: active ? 'var(--accent)' : 'var(--text-2)', boxShadow: active ? '0 2px 8px rgba(0,0,0,0.1)' : 'none' }}>
                {t.label}
              </button>
            )
          })}
