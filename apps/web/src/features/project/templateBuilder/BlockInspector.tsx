...
import { TYPE_LABEL } from './blocks'
import type { CanvasBlock, CanvasCharacter, CanvasTemplate } from '@/components/TemplateCanvas'
...
export type InspectorCtx = {
  ...
  updateBlock: (id: string, key: string, val: unknown) => void
  updateBlockStyle: (id: string, key: string, val: unknown) => void
  setCols: (id: string, n: number) => void
  setColWidth: (id: string, i: number, pct: number) => void
  addTag: (blockId: string) => void
  updateTag: (blockId: string, tagId: string, key: string, val: unknown) => void
...
function seg(active: boolean) {
  return {
    flex: 1, fontFamily: 'inherit', fontSize: 12, fontWeight: 700, border: 'none',
    cursor: 'pointer', borderRadius: 8, padding: '7px 4px',
    background: active ? 'var(--accent)' : 'transparent',
    color: active ? '#fff' : 'var(--text-2)', whiteSpace: 'nowrap' as const,
  }
}
...
const textInput = {
  width: '100%', boxSizing: 'border-box' as const, fontFamily: 'inherit', fontSize: 13.5,
  lineHeight: 1.6, color: 'var(--text)', background: 'var(--bg-3)', border: '1px solid var(--border)',
  borderRadius: 9, padding: '8px 10px', outline: 'none',
}

function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return <div><div style={labelStyle}>{title}</div>{children}</div>
}
...
function Segmented<T extends string | number>({ value, options, onPick }: { value: T; options: { v: T; l: string }[]; onPick: (v: T) => void }) {
  return (
    <div style={{ display: 'flex', gap: 4, background: 'var(--bg-3)', padding: 4, borderRadius: 10, flexWrap: 'wrap' }}>
      {options.map(o => (
        <button key={String(o.v)} style={seg(value === o.v)} onClick={() => onPick(o.v)}>{o.l}</button>
      ))}
    </div>
  )
}

function Swatches({ value, lib, allowTransparent, onPick }: { value?: string; lib: string[]; allowTransparent?: boolean; onPick: (hex: string) => void }) {
  const chip = (sel: boolean) => ({
    width: 30, height: 30, borderRadius: 9, cursor: 'pointer', padding: 0, flexShrink: 0,
    border: sel ? '2px solid var(--accent)' : '1px solid var(--border)',
  })
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
      {allowTransparent ? (
        <button title="透明" onClick={() => onPick('transparent')} style={{ ...chip(value === 'transparent' || !value), background: 'repeating-conic-gradient(#ccc 0% 25%, #fff 0% 50%) 50% / 12px 12px' }} />
      ) : null}
      {lib.map(hex => <button key={hex} title={hex} onClick={() => onPick(hex)} style={{ ...chip(value === hex), background: hex }} />)}
      <label style={{ ...chip(false), position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-2)', background: 'var(--bg-2)', borderStyle: 'dashed' }}>
        ...
        <input type="color" value={value && value[0] === '#' ? value : '#888888'} onChange={e => onPick(e.target.value)} style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer' }} />
      </label>
    </div>
  )
}

const BORDER_W = [{ v: 0, l: '無' }, { v: 1, l: '細' }, { v: 2, l: '中' }, { v: 3, l: '粗' }]
const BORDER_S = [{ v: 'solid', l: '實線' }, { v: 'dashed', l: '虛線' }, { v: 'dotted', l: '點線' }, { v: 'double', l: '雙線' }]

export function BlockInspector({ block, ctx }: { block: CanvasBlock; ctx: InspectorCtx }) {
  const { character, activeTemplate, selectBlock, moveBlockDir, duplicateBlock, removeBlock, updateBlock, updateBlockStyle, setCols, setColWidth, addTag, updateTag, removeTag, libColors, moveOut } = ctx
  const [showImgPicker, setShowImgPicker] = useState(false)
  const st = block.style || {}
  const t = block.type
  const lib = Array.from(new Set([...(character.palette || []).map(p => p.hex).filter(Boolean), ...libColors()]))
  ...
  return (
    ...
    {showImgPicker && (
      <AssetPickerModal
        onSelect={url => { updateBlock(block.id, 'popupImg', url); setShowImgPicker(false) }}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 12, color: 'var(--text-2)' }}>選取：<b style={{ color: 'var(--accent)' }}>{TYPE_LABEL[t] ?? t}</b></span>
        <div style={{ display: 'flex', gap: 5 }}>
          ...
          <button
            title="刪除"
            style={{ ...iconBtn, border: '1px solid #e0b3ad', background: '#fbeeec', color: '#c0584f' }}
            onMouseEnter={e => { const b = e.currentTarget; b.style.background = '#c0584f'; b.style.color = '#fff'; b.style.borderColor = '#c0584f' }}
            onMouseLeave={e => { const b = e.currentTarget; b.style.background = '#fbeeec'; b.style.color = '#c0584f'; b.style.borderColor = '#e0b3ad' }}
        ...
        <>
          <Group title="資料來源（區塊）">
            <select value={block.sourceId || ''} onChange={e => updateBlock(block.id, 'sourceId', e.target.value)} style={selectStyle}>
              ...
              {(character.sections || []).map(s => <option key={s.id} value={s.id}>{(s as { title?: string }).title || '未命名區塊'}</option>)}
            </select>
          </Group>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            ...
            <button onClick={() => updateBlock(block.id, 'hideTitle', !block.hideTitle)} style={{ flexShrink: 0, width: 46, height: 26, borderRadius: 999, border: 'none', cursor: 'pointer', background: block.hideTitle ? 'var(--accent)' : 'var(--border)', position: 'relative' }}>
              <span style={{ position: 'absolute', top: 3, left: block.hideTitle ? 23 : 3, width: 20, height: 20, borderRadius: '50%', background: '#fff' }} />
            </button>
          </div>
      ...
      {t === 'album' ? (
        <>
          <Group title="資料來源（相簿）">
            <select value={block.sourceId || ''} onChange={e => updateBlock(block.id, 'sourceId', e.target.value)} style={selectStyle}>
              <option value="">（未綁定）</option>
              {(character.albums || []).map(a => <option key={a.id} value={a.id}>{(a as { name?: string }).name || '未命名相簿'}</option>)}
            </select>
          </Group>
          ...
          {t === 'popup' ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                <input value={block.popupImg || ''} onChange={e => updateBlock(block.id, 'popupImg', e.target.value)} placeholder="圖片 URL（選填）" style={{ ...textInput, flex: 1 }} />
                <button type="button" onClick={() => setShowImgPicker(true)} title="從圖庫選圖" style={{ flexShrink: 0, width: 36, height: 36, borderRadius: 9, border: '1px solid var(--border)', background: 'var(--bg-2)', color: 'var(--accent)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {t === 'columns' ? (
        <>
          <Group title="欄數"><Segmented value={block.cols || 2} options={[{ v: 2, l: '2 欄' }, { v: 3, l: '3 欄' }, { v: 4, l: '4 欄' }]} onPick={v => setCols(block.id, v as number)} /></Group>
          <Group title="各欄寬度">
      ...
      {['heading', 'tagline', 'avatar', 'cover', 'text'].includes(t) ? (
