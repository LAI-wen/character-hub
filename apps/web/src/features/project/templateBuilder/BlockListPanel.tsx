...
import type { CanvasBlock, CanvasTemplate } from '@/components/TemplateCanvas'
...
export type ListCtx = {
  ...
  moveColChild: (id: string, dir: -1 | 1) => void
}
...
function RowPill({ block, label, selected, canUp, canDown, canLeft, canRight, onSelect, onUp, onDown, onLeft, onRight, onDup, onDel }: {
  block: CanvasBlock; label: string; selected: boolean
  canUp: boolean; canDown: boolean; canLeft?: boolean; canRight?: boolean
  onSelect: () => void; onUp: () => void; onDown: () => void
  onLeft?: () => void; onRight?: () => void; onDup: () => void; onDel: () => void
}) {
  const stop = (fn: () => void, enabled = true) => (e: React.MouseEvent) => { e.stopPropagation(); if (enabled) fn() }
  const compact = !!onLeft
  const buttons = (
    <>
      ...
      <button
        title="刪除"
        style={{ ...miniBtn, color: '#c0584f', borderColor: '#e0b3ad' }}
        onMouseEnter={e => { const b = e.currentTarget; b.style.background = '#c0584f'; b.style.color = '#fff'; b.style.borderColor = '#c0584f' }}
        onMouseLeave={e => { const b = e.currentTarget; b.style.background = 'var(--bg-2)'; b.style.color = '#c0584f'; b.style.borderColor = '#e0b3ad' }}
        onClick={stop(onDel)}
  ...
  if (compact) {
    return (
      <div onClick={onSelect} style={{ display: 'flex', flexDirection: 'column', gap: 7, padding: 9, borderRadius: 12, border: selected ? '1px solid var(--accent)' : '1px solid var(--border)', background: selected ? 'var(--accent-soft)' : 'var(--bg-2)', cursor: 'pointer' }}>
  ...
  return (
    <div onClick={onSelect} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 8px 7px 13px', borderRadius: 999, border: selected ? '1px solid var(--accent)' : '1px solid var(--border)', background: selected ? 'var(--accent-soft)' : 'var(--bg-2)', cursor: 'pointer' }}>
      <Icon name={TYPE_ICON[block.type] || 'list'} size={14} style={{ color: 'var(--text-2)' }} />
...
export function BlockListPanel({ ctx }: { ctx: ListCtx }) {
  const { selBlock, activeTemplate, selectBlock, moveBlockDir, duplicateBlock, removeBlock, moveColChild } = ctx
  const blocks = activeTemplate?.blocks || []

  if (!blocks.length) {
    return <div style={{ border: '1.5px dashed var(--border)', borderRadius: 14, padding: 28, textAlign: 'center', color: 'var(--text-2)', fontSize: 13 }}>還沒有積木，先到「加入」加一個。</div>
  }
        ...
        {blocks.map((b, i) => {
          ...
          return (
            <div key={b.id} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <RowPill
                block={b}
                label={isCol ? `佈局（${cols.length} 欄）` : TYPE_LABEL[b.type] || b.type}
                selected={selBlock === b.id}
                ...
                <div style={{ display: 'flex', gap: 6, paddingLeft: 14 }}>
                  {cols.map((col, ci) => (
                    <div key={ci} style={{ flex: 1, minWidth: 0, background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: 12, padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      {col.length ? col.map((cb, cj) => (
                        <RowPill
                          key={cb.id} block={cb}
                          label={TYPE_LABEL[cb.type] || cb.type}
                          selected={selBlock === cb.id}
                          canUp={cj > 0} canDown={cj < col.length - 1}
                          canLeft={ci > 0} canRight={ci < cols.length - 1}
